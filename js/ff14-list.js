...
function custom_recent() {
  ...
  $(".entry__block__wrapper")
    ...
    .each(function(index, item) {
      ...
      // ワールドタグ
      var world = '<span class="world">' + array[index].world + "</span>";
      // コメントタグ
      var comment =
        "<span>💬 " +
        (array[index].comment !== void 0 ? array[index].comment : "0") +
        "</span>";
      // いいねタグ
      var like =
        "<span>💗 " +
        (array[index].like.length !== 0 ? array[index].like : "0") +
        "</span>";
      // 投稿時刻タグ
      var time =
        '<span class="time"><time>' + array[index].date + "</time></span>";
      // 題名タグ
      var title = "<h5>" + array[index].title + "</h5>";
      ...
      var html =
        ...
        like +
        "</div>";
      html = '<a href="' + array[index].url + '">' + html + "</a>";
      item.innerHTML = html;
    });
}
