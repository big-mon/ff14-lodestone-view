...
function custom_recent() {
  ...
  $(".entry__block__wrapper")
    ...
    .each(function(index, item) {
      ...
      // ワールドタグ
      var world = '<span class="world">' + entry.world + "</span>";
      // コメントタグ
      var comment =
        "<span>💬 " +
      ...
      // いいねタグ
      var like =
        "<span>💗 " + (entry.like.length !== 0 ? entry.like : "0") + "</span>";
      // 投稿時刻タグ
      var time = '<span class="time"><time>' + entry.date + "</time></span>";
      // 題名タグ
      var title = "<h5>" + entry.title + "</h5>";
