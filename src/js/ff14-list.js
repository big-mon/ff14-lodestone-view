...
let LIST_FUNCTION = {
  ...
  editPopular: function() {
    ...
    // 新規スライダーを作成
    $(".heading--pickup").after('<div id="custom-slider" class="owl-carousel owl-theme"></div>');
  ...
  editRecent: function() {
    ...
    $(".entry__block__wrapper")
      ...
      .each(function(index, item) {
        // ◆オリジナルのソースから情報を抽出
        let entry = {
          title: $(item).find(".entry__blog_block__title").text(),
          img: $(item).find(".entry__blog_block__img img").attr("src"),
          date: $(item).find(".datetime_dynamic_ymdhm").text(),
          comment: $(item).find(".entry__blog_block__header__comment").text(),
          like: $(item).find(".entry__blog_block__header__like").text(),
          world: $(item).find(".entry__blog_block__search__chara__world").text(),
          tags: $(item).find(".entry__blog_block__tag").text(),
          url: $(item).find(".entry__blog_block__title").attr("href"),
          author: $(item).find(".entry__blog_block__search__chara__name").attr("href").match(/^.+character\/([0-9]+)\//)[1]
        };
        ...
        let tags = {
          img:
            ...
            + '" alt="' + entry.title + '" width="300" height="168" loading="lazy" />',
          world: '<span class="world">' + entry.world + "</span>",
          comment: "<span>💬 " + (entry.comment !== void 0 ? entry.comment : "0") + "</span>",
          like: "<span>💗 " + (entry.like.length !== 0 ? entry.like : "0") + "</span>",
        ...
        // ◆個別記事カード内のタグを再構成
        let html =
          '<a href="' + entry.url + '" data-character="' + entry.author + '">'
          + '<figure class="eyecatch">' + tags.img + tags.world + "</figure>"
          + tags.title
          + '<div class="info">'
          + tags.time
          + tags.comment
